	...
	for _, tag := range m.unknownFieldTags() {
		...
		for _, uf := range ufs {
			...
			if uf.Encoding == proto.WireStartGroup {
				...
				err = marshalUnknownGroup(b, in, true)
				if err != nil { return err }
				err = b.end()
				if err != nil { return err }
				err = b.WriteByte('}')
...
func marshalKnownFieldText(b *indentBuffer, fd *desc.FieldDescriptor, v interface{}) error {
	...
	case reflect.Float32, reflect.Float64:
		_, err := b.WriteString(strconv.FormatFloat(rv.Float(), 'g', -1, 64))
		return err
	case reflect.Bool:
...
func marshalUnknownGroup(b *indentBuffer, in *codedBuffer, topLevel bool) error {
	first := true
	for {
		if in.eof() {
			if topLevel {
		...
		if wireType == proto.WireStartGroup {
			...
			err = b.start()
			if err != nil { return err }
			err = marshalUnknownGroup(b, in, false)
			if err != nil { return err }
			err = b.end()
			if err != nil { return err }
			err = b.WriteByte('}')
